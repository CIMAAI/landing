...
(() => {
  ...
  clientKeys.filter(clientKey => !clientKey.includes('_')).forEach(clientKey => {
    ...
    execSync(`git config --local user.email "action@github.com" && git config --local user.name "GitHub Action" && npx gh-pages --repo git@github.com:CIMAAI/${clientKey}.com.git --dist _site/${clientKey} -t`);
  });
})();